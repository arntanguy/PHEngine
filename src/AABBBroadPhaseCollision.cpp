...
AABBBroadPhaseCollision::AABBBroadPhaseCollision(PhysicsWorld *phWorld) :
		BroadPhaseCollision(phWorld) {
}
...
#pragma omp section
	...
	for (short axis = 0; axis < 3; axis++) {
		...
		for (sortedListIt = sortedList[axis].begin();
				sortedListIt != sortedList[axis].end(); sortedListIt++) {
			/*            // Pedicate min*/
			RemoveFromActiveList p(sortedListIt->min);
			activeList[axis].erase(
					std::remove_if(activeList[axis].begin(),
							activeList[axis].end(), p), activeList[axis].end());
			...
			if (activeList[axis].size() > 1) {
				...
				for (; i < activeList[axis].size(); i++) {
					...
					while (j < activeList[axis].size()) {
						if (j != i) {
							RigidBodyPair currentPair = RigidBodyPair(
									activeList[axis].at(i).rigidBody,
									activeList[axis].at(j).rigidBody);
							std::unordered_map<RigidBodyPair, AxisCollide>::iterator cit =
									collidingPairs.find(currentPair);
